...
func doReduce(
	...
	//
	out,err := os.OpenFile(outFile, os.O_CREATE|os.O_RDWR|os.O_APPEND,0666)
	if err != nil {
		debug("fail to open outfile:%v",err)
		return
	}
	defer out.Close()
	...
	for i:=0;i<nMap;i++ {
		...
		if err != nil {
			debug("fail to openfile:%v %v",err,name)
			return
		}
		defer tmpFile.Close()
		decode := json.NewDecoder(tmpFile)
